...
exports.getCachedLandlordInsight = exports.landlordAnalyticsAiGateway = exports.getLandlordAnalyticsAiInput = exports.getListingAnalyticsSummary = exports.getListingMetrics = exports.onAnalyticsEventCreate = exports.ANALYTICS_EVENT_TYPES = void 0;
exports.processAnalyticsEvent = processAnalyticsEvent;
exports.buildLandlordAnalyticsAiPayload = buildLandlordAnalyticsAiPayload;
 ...
 */
async function buildLandlordAnalyticsAiPayload(landlordId, now = new Date()) {
 ...
 */
exports.getLandlordAnalyticsAiInput = functions.https.onCall(async (data, context) => {
    const auth = context.auth;
    if (!auth?.uid) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
    }
...
function getGroqApiKey() {
    try {
        const fromConfig = functions.config().groq?.api_key;
...
function containsPromptInjection(query) {
    const lower = query.toLowerCase();
    return INJECTION_PATTERNS.some((p) => lower.includes(p));
}
function sanitizeLandlordMessage(message) {
...
function isValidLandlordAnalyticsResponse(response) {
    const lower = response.toLowerCase();
    return LANDLORD_AI_RESPONSE_KEYWORDS.some((k) => lower.includes(k));
}
async function callGroqFromGateway(apiKey, systemPrompt, userContent) {
...
async function getCachedInsight(uid) {
    ...
    if (typeof reply !== "string" || !reply || !at)
        return null;
    const atMs = at?.toMillis?.() ?? 0;
    if (Date.now() - atMs > LANDLORD_AI_CACHE_MAX_AGE_MS)
        return null;
    return { lastReply: reply, generatedAt: new Date(atMs).toISOString() };
}
async function setCachedInsight(uid, lastReply, contextHash, messageHash) {
 ...
 */
exports.landlordAnalyticsAiGateway = functions.https.onCall(async (data, context) => {
    const auth = context.auth;
    if (!auth?.uid) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
    }
 ...
 */
exports.getCachedLandlordInsight = functions.https.onCall(async (data, context) => {
    const auth = context.auth;
    if (!auth?.uid) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
    }
